"""
Final MoE Verification Script - CNN "Eyes" Patch.

This script validates the fix for the "Zero Weight" CNN issue.

ROOT CAUSE: CNN was receiving PCA latents (tabular) instead of time-series data.
SOLUTION: Feed frac_diff (memory-preserving price series) directly to CNN.

Pipeline:
1. Load Data
2. Calculate FracDiff (memory preservation)
3. Apply TensorFlex (feature refinement)
4. Train MoE with cnn_source_feature="frac_diff"

Success Criteria:
- Pattern Expert Weight > 5% (CNN gets non-zero weight)
- Total Precision > 53%
- Expectancy > 0.0

Goal: Give CNN "eyes" (raw time series) so Gating Network recognizes its value.
"""
...
def run_specialized_moe_pipeline(
    ...
    if cnn_weight_pass:
        ...
        print("   Check that frac_diff column is being passed correctly")
